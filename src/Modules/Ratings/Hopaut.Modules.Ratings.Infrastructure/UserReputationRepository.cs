...
public sealed class UserReputationRepository : IUserReputationRepository
{
    ...
    public Task<UserReputation?> GetAsync(string userId, CancellationToken ct = default)
        => _db.UserReputations.FirstOrDefaultAsync(r => r.UserId == userId, ct);

    public async Task<Dictionary<string, UserReputation>> GetBatchAsync(IEnumerable<string> userIds, CancellationToken ct = default)
    {
    ...
    public async Task UpsertAsync(string userId, CancellationToken ct = default)
    {
